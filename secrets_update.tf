resource "null_resource" "update_rds_secret" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      aws secretsmanager put-secret-value --secret-id my-db-secret --secret-string '{"connection_string": "postgres://mydbuser:P@55w0rd@'"$endpoint"':5432/dotlanches", "db_password": "P@55w0rd"}'
    EOT
